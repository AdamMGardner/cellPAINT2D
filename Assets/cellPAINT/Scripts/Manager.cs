...
public class Manager : MonoBehaviour {
    ...
    public void changeDescription(GameObject another, SpriteRenderer otherImage) {
        ...
        Debug.Log("changeDescription "+props.name);
        ...
        if (props.is_fiber && props.name.Contains("Membrane"))
        {
            textDescription.text += "<br>(note: draw clockwise)";
        }
    ...
    public void changeColorMaterial(Color32 color)
    {
        ...
        if (current_prefab!=null){
            if (drawMode) current_prefab.GetComponent<SpriteRenderer>().sharedMaterial.color = color;
        }
    ...
    public void BuildPrefab2D(string aname = null)
    {
        ...
        if (inde < sprites_names.Count) img_name = sprites_names[inde];
    ...
    public void restoreOneInstance(string name, Vector3 pos, float Zangle, int zorder,
        bool surface = false, bool pinned = false)
    {
        GameObject Prefab;
        if (!all_prefab.ContainsKey(name))
        {
    ...
    {
        if (rbCount >= 5000) warningPanel.SetActive(true);
        if (rbCount >= 5000) return;
        ...
        GameObject other = null, GameObject aprefab = null)
    {
        ...
        fiber_quadOb.transform.position = pos;
            
        if (props.fiber_Middle)
        {
            fiber_quadOb.transform.position = new Vector3 (fiber_quadOb.transform.position.x, fiber_quadOb.transform.position.y, 0.125f);
        }
        else if (props.fiber_Bottom)
        {
            fiber_quadOb.transform.position = new Vector3 (fiber_quadOb.transform.position.x, fiber_quadOb.transform.position.y, 0.25f);
        }
        else
        {
            fiber_quadOb.transform.position = new Vector3 (fiber_quadOb.transform.position.x, fiber_quadOb.transform.position.y, 0.0f);
        }
    ...
    public void AddFiberParent(string name) {
    ...
    {
        float delta = Vector3.Distance(prev_mousePos, transform.position);
        bool input_event = (Input.GetMouseButton(0) && delta > 0.005f) || Input.GetMouseButtonDown(0);
        int start = everything.Count;
        if (input_event)
        {
    ...
    void DestroyHierarchyFamily(string name)
    {
        var family =
                       from o in Manager.Instance.root.transform.GetComponentsInChildren<Transform>()
                       where o.name.Contains(name)
    ...
    void DestroyHierarchy(Transform parent)
    {
        if (!parent) return;
    ...
    void pindragInstance() {
        ...
        else
        {
            ...
            else if (dragMode) { dragInstance(other, hit.point); togglePinOutline(true); }
            else if (eraseMode)
            {
    ...
    void Update() {
        ...
        else if (bucketMode) fillCompartments();
        else { }
    ...
    */
    ...
    private void CleanOutline()
    {
        if (below && below != attach1)
        {
            ...
            if (below.transform.parent.GetComponent<PrefabProperties>() != null)
            {
