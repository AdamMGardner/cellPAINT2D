...
public class FileBrowser2
{
    ...
    protected bool save_dialog;
    public string m_textPath="sample.txt";
    // Browsers need at least a rect, name and callback
    public FileBrowser2(Rect screenRect, string name, bool save,FinishedCallback callback)
    {
        m_name = name;
    ...
    void DoMyWindow(int windowID) {
        ...
        //I added a ton of layout event checks. It fixewd the errors, but the code doesn't work... -Adam.
        ...
        var complete_path = (m_selectedFile == -1) ? "" : Path.Combine(m_currentDirectory, m_files[m_selectedFile]);
        if (save_dialog)
        {
            //m_textPath = (m_selectedFile == -1) ? "" : m_files[m_selectedFile];
            if (Event.current.type == EventType.Layout)
            {
                m_textPath = GUILayout.TextField(m_textPath, GUILayout.Width(400), GUILayout.ExpandWidth(true));
                complete_path = Path.Combine(m_currentDirectory, m_textPath);
            }
        }
        else {
            GUILayout.Label(complete_path ?? "none selected");
        }
        if (Event.current.type == EventType.Layout)
        {
