...
namespace GracesGames.SimpleFileBrowser.Scripts {
	...
	public class DemoCaller : MonoBehaviour {
        ...
        public void Doit(bool saving) {
            OpenFileBrowser(saving ? FileBrowserMode.Save : FileBrowserMode.Load, null);
        }
		// Open the file browser using boolean parameter so it can be called in GUI
		public void OpenFileBrowser(bool saving, Action<string> callback) {
			OpenFileBrowser(saving ? FileBrowserMode.Save : FileBrowserMode.Load, callback);
		}

		// Open a file browser to save and load files
		private void OpenFileBrowser(FileBrowserMode fileBrowserMode, Action<string> callback) {
			...
			if (fileBrowserMode == FileBrowserMode.Save) {
				fileBrowserScript.SaveFilePanel("NewFile", FileExtensions);
				// Subscribe to OnFileSelect event (call SaveFileUsingPath using path) 
				fileBrowserScript.OnFileSelect += callback;
			} else {
				fileBrowserScript.OpenFilePanel(FileExtensions);
                // Subscribe to OnFileSelect event (call LoadFileUsingPath using path) 
                fileBrowserScript.OnFileSelect += callback;// LoadFileUsingPath;
			}
		}
